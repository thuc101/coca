...
export default {
  ...
  state: {
    items: [],        // page hiện tại
    rawItems: [],     // toàn bộ data (quan trọng cho search local)
  ...
  actions: {
    ...
    //   // 👉 lấy data từ DB (FULL LIST)
    //   const res = await dbService.getAll('SanPham')
    ...
    async LoadPaging({ commit, state }, payload = {}) {
      ...
      // 🔥 LẤY TOÀN BỘ TABLE
      const allData = await dbService.getAll('SanPham')

      commit('SET_RAW', allData)

      let data = allData

      // 🔍 SEARCH TOÀN BỘ TABLE
      if (keyword && keyword.trim() !== '') {
        const kw = keyword.toLowerCase()
    ...
    async Insert({ dispatch }, item) {
      await dbService.insert('SanPham', item)
      dispatch('LoadPaging')
    },
    ...
    async Update({ dispatch }, item) {
      await dbService.update('SanPham', item)
      dispatch('LoadPaging')
    },
    ...
    async Delete({ dispatch }, id) {
      await dbService.delete('SanPham', id)
      dispatch('LoadPaging')
    }
